...
public:
	...
	{}
		...
		SLATE_ARGUMENT(bool, RestrictedTags)// If we are dealing with restricted tags or regular message tags
	SLATE_END_ARGS();
...
private:
	...
	TSharedPtr<SComboBox<TSharedPtr<FString> > > TagSourcesComboBox;

	/** Callback for when a new message tag has been added to the INI files */
	FOnMessageTagAdded OnMessageTagAdded;

	/** Callback to see if the message tag is valid. This should be used for any specialized rules that are not covered by IsValidMessageTagString */
